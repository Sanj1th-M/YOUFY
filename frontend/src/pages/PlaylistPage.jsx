...
// getPlaylistVideos returns VideoDetailed[] which has videoId, name, artist, duration, thumbnails
...
export default function PlaylistPage() {
  ...
  useEffect(() => {
    ...
    getPlaylistSongs(playlistId)
      .then(data => {
        ...
        // Backend merges getPlaylist + getPlaylistVideos → data.videos
      ...
      .catch(() => {
        if (!cancelled) setError('Could not load playlist. Please try again.');
      })
      .finally(() => {
  ...
  const authorName   = playlist?.artist?.name || 'Unknown';
  const thumbnail    = getBestThumbnail(playlist?.thumbnails) || '/logo-dark.png';
  const trackCount   = songs.length || playlist?.videoCount || 0;
    ...
    <div className="px-4 md:px-6 lg:px-8 py-6 max-w-5xl mx-auto pb-32">
      ...
      {songs.length > 0 ? (
        ...
        <div className="flex flex-col items-center py-16 gap-3 text-gray-500">
          <p className="text-sm">No tracks found for this playlist</p>
        </div>
      )}
    </div>
  );
